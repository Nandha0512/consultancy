...
import { getNammakal, updateNammakals } from '../../../../actions/ProductActions/Nactions';
...
const ProductsUpdate = () => {
    ...
    const { isNammakalUpdated, Nammakal } = useSelector(state => state.NammakalState)
    ...
    const submitHandler = (e) => {
        ...
        dispatch(updateNammakals(userId, formData))
    }

    useEffect(() => {
        dispatch(getNammakal(userId))
    }, [isNammakalUpdated, dispatch])


    useEffect(() => {
        if (Nammakal._id) {
            setName(Nammakal.name);
            setDescription(Nammakal.description);
            setPrice(Nammakal.price);
            setStocks(Nammakal.stocks)
        }
    }, [Nammakal])
